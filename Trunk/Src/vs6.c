...
int makeVs6Scripts()
{
	...
	if (csharp && getenv("DOTNET_PATH") == NULL)
	{
		...
		if (strcmp(csc, "csc.exe") == 0)
			printf("This will be something like C:\\Windows\\Microsoft.NET\\Framework\\v1.0.3705.\n\n");
	}
#endif
...
static int writeWorkspace()
{
	...
	for (i = 0; i < project->numPackages; ++i)
	{
		...
		fprintf(file, "###############################################################################\n");
		fprintf(file, "\n");
		fprintf(file, "Project: \"%s\"=%s%s.dsp\n", replaceChars(package->name,"-"), reversePath(project->path,package->path,WINDOWS,1), package->name);
		fprintf(file, "\n");
...
static int writeVcProject(Package* package)
{
	...
	for (i = 0; i < package->numConfigs; ++i)
	{
		...
		fprintf(file, "# PROP Output_Dir \"");
			fprintf(file, reversePath(package->path, outdir, WINDOWS, 0));
			insertPath(file, getDirectory(config->target, 0), WINDOWS, 1);
			fprintf(file, "\"\n");
		fprintf(file, "# PROP Intermediate_Dir \"%s\\%s\"\n", translatePath(package->objdir,WINDOWS), config->name);
		if (strcmp(package->kind, "lib") != 0)
			fprintf(file, "# PROP Ignore_Export_Lib %d\n", importlib ? 0 : 1);
		fprintf(file, "# PROP Target_Dir \"\"\n");

		fprintf(file, "# ADD BASE CPP /nologo /W%d", warnings);
		if (debug)  
			fprintf(file, " /Gm /GX /ZI /Od /D \"_DEBUG\" /YX /FD /GZ /c\n");
		else
			fprintf(file, " /GX /O2 /D \"NDEBUG\" /YX /FD /c\n");
			
		fprintf(file, "# ADD CPP /nologo %s /W%d", runtime, warnings);
		...
		writeList(file, config->includePaths, " /I \"", "\"", "", convertPath, NULL);
		if (debug)
			fprintf(file, " /YX /FD /GZ /c\n");
		else
			fprintf(file, " /YX /FD /c\n");
		...
		if (strcmp(package->kind, "lib") != 0)
		{
			...
			fprintf(file, "# ADD LINK32");
				...
				fprintf(file, " /out:\"%s", reversePath(package->path, prjCfg->bindir, WINDOWS, 1));
				fprintf(file, "%s.%s\"", translatePath(config->target,WINDOWS), targetext);
				...
				fprintf(file, " /pdbtype:sept /libpath:\"%s\"", reversePath(package->path, prjCfg->libdir, WINDOWS, 0));
				writeList(file, config->libPaths, " /libpath:\"", "", "\"", NULL, NULL);
				if (debug) {
					fprintf(file, " /pdb:\"%s", reversePath(package->path, prjCfg->bindir, WINDOWS, 1));
					fprintf(file, "%s.pdb\" /debug", translatePath(config->target,WINDOWS));
				}
		...
		else
		{
			...
			fprintf(file, "# ADD LIB32 /nologo /out:\"%s", reversePath(package->path, prjCfg->libdir, WINDOWS, 1));
				fprintf(file, "%s.%s\"\n", translatePath(config->target,WINDOWS), targetext);
		}
	}
...
static int writeCsProject(Package* package)
{
	...
	for (i = 0; i < package->numConfigs; ++i)
	{
		...
		fprintf(file, "# PROP Output_Dir \"");
			fprintf(file, reversePath(package->path, project->binaries, WIN32, 1));
			insertPath(file, getDirectory(config->target), WIN32, 1);
			fprintf(file, "\"\n");
		fprintf(file, "# PROP Intermediate_Dir \"%s\\%s\"\n", translatePath(package->objdir,WIN32), config->name);
		fprintf(file, "# PROP Target_Dir \"\"\n");
		fprintf(file, "# Begin Special Build Tool\n");
		fprintf(file, "SOURCE=\"$(InputPath)\"\n");

		fprintf(file, "PostBuild_Cmds=$(DOTNET_PATH)\\%s /nologo", csc);
			fprintf(file, " /out:\"%s", reversePath(package->path,project->binaries,WIN32,1));
			...
			fprintf(file, " /lib:\"%s\"", reversePath(package->path, project->binaries, WIN32, 1));
			writeList(file, config->links, " /r:", ".dll", "", checkRef, &i);
			fprintf(file, " %s\\%s\\*.cs\n", package->objdir, config->name);
